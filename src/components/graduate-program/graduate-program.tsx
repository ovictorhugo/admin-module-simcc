      ...
      {isModalOpen && (
        <>
          {programSelecionado.length == 0 ? (
            <main className=" gap-4 md:gap-8 flex flex-col md:p-8 pt-0 md:pt-0 w-full">
              <div className="bg-cover bg-bottom bg-no-repeat" style={{ backgroundImage: `url(${bg_graduate})` }}>
                <div className="justify-center m w-full  flex max-w-[980px] flex-col items-center lg:items-start  gap-2 py-8 md:py-12 md:pb-8 lg:py-24 lg:pb-20" >
                  <Link
                    to={'/informacoes'}
                    className="
                      inline-flex z-[2] w-fit items-center rounded-lg
                      bg-neutral-100 dark:bg-neutral-700
                      gap-2  px-3 py-1 text-sm font-medium
                    "
                  >
                    <Info size={12} />
                  ...
                  <h1
                    className={`
                      z-[2] lg:text-left text-center
                      ${movel ? 'text-[1.4rem] w-[80%]' : 'text-4xl max-w-[600px]'} font-bold
                      leading-tight tracking-tighter 
                      md:text-5xl lg:leading-[1.1]
                      md:block mb-4 
                    `}
                  >
                  ...
                  <Alert className={`h-14 mt-8 p-2 flex items-center justify-between lg:max-w-[600px] lg:w-[60vw] ${movel ? 'w-[85%] mt-1' : 'w-full'} `}>

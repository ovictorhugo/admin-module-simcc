    ...
    <div className=" items-center flex flex-col ">
      <div className="bg-cover bg-no-repeat bg-center w-full">
        ...
        <div className="justify-center px-4 md:px-8 w-full mx-auto flex max-w-[1200px] flex-col items-center gap-2 py-8 md:py-12 md:pb-8 lg:py-24 lg:pb-20" >
          ...
          <h1 className={`${movel ? "text-[1.4rem]" : "text-3xl"} z-[2] text-center max-w-[980px] font-bold leading-tight tracking-tighter md:text-5xl lg:leading-[1.1]  md:block mb-4 `}>
            Experimente{" "}
            <strong className={`bg-eng-blue  rounded-md px-3 ${movel ? "pb-1" : "pb-2"} text-white font-medium`}>
              {" "}
              pesquisar um tema
            </strong>{" "}
      ...
      <div className={`${movel && "-mt-4"} w-full md:px-8 md:gap-8 gap-4 flex flex-col px-4`}>
        ...
        <Alert className="grid gap-3 lg:grid-cols-4 grid-cols-2">
          <Link onClick={() => onOpenResult('articles-home')} to={'/resultados?type_search=article&terms=&researcher=false'}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0">
              <div>
                <CardTitle className={`${movel ? "text-[0.7rem]" : ""} font-medium`}>
                  Total de artigos
                </CardTitle>
              </div>

              <Quotes className={`${movel ? "w-3 h-3" : "h-4 w-4"}  text-muted-foreground`} />
          ...
          <Link onClick={() => onOpenResult('book-home')} to={'/resultados?type_search=book&terms=&researcher=false'}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Book className={`${movel ? "w-3 h-3" : "h-4 w-4"}  text-muted-foreground`} />
          ...
          <Link onClick={() => onOpenResult('book-home')} to={'/resultados?type_search=book&terms=&researcher=false'}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Books className={`${movel ? "w-3 h-3" : "h-4 w-4"}  text-muted-foreground`} />
          ...
          <Link onClick={() => onOpenResult('patent-home')} to={'/resultados?type_search=patent&terms=&researcher=false'}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Copyright className={`${movel ? "w-3 h-3" : "h-4 w-4"}  text-muted-foreground`} />
        ...
        <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-3 mb-5">
          {version ? (
            <div className="h-full gap-8 grid">
              <Link to={'/listagens'}>
                <Alert className="p-0 -mb-4">
                  <CardHeader className="flex p-10 flex-row items-center justify-between space-y-0">
                    <div>
              ...
              <Link to={'/listagens'}>
                <Alert className="p-0 ">
                  <CardHeader className="flex flex-row p-10 items-center justify-between space-y-0">
                    <div>
                      <CardTitle className="text-sm font-medium">
                ...
                <CardHeader className="flex p-0 flex-row items-center justify-between space-y-0 ">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger> <Info className="h-4 w-4 text-muted-foreground" /></TooltipTrigger>
                        <TooltipContent className={`${movel && "right-5 bottom-4 relative"}`}>
                          {
                            movel
                              ?
                              <p className="text-[0.7rem]">
                                Essas informações não representam <br /> a produção total da Escola desde a sua fundação.<br />É um recorte a partir de {year}
                              </p>
                              :

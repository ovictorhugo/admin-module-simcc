import { AppWindow, Book, BookOpen, CalendarBlank, Copyright, File, Graph, LinkBreak, PenNib, Quotes } from "phosphor-react";
...
const {searchType, valoresSelecionadosExport, valorDigitadoPesquisaDireta} = useContext(UserContext)
    ...
    return (
        <div className="flex  w-full" >
                    ...
                    <div
                      className={`h-full w-2 rounded-l-md dark:border-neutral-800 border border-neutral-200 border-r-0  ${props.type == 'livro' && ('bg-pink-800')} ${props.type == 'software' && ('bg-cyan-400')} ${props.type == 'marca' && ('bg-cyan-600')} ${props.type == 'patente' && ('bg-cyan-800')}   ${props.type == 'capLivro' && ('bg-pink-300')}`}
                    > 
                    </div>
...
{props.type == 'patente' && (
                             <h3 className="font-semibold mb-4 ">{props.id}</h3>
                          )}
...
{props.type == 'patente' && (
                            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center"><Copyright size={12}/>{props.financing}</div>
                        )}

      ...
      <div className="flex items-center flex-col relative">
        <ScrollArea>
          <div className={`mb-4 flex gap-3 overflow-x-scroll items-center flex-wrap  ${movel ? " flex-col justify-start h-12 text-[0.65rem]" : "justify-center"}`}>
            ...
            {props.graduation != '' && (
              <div className={`bg-blue-700 py-2 px-4 text-white rounded-md font-bold flex gap-2 items-center`}><GraduationCap size={12} className="textwhite" /> {props.graduation}</div>
            )}
            {props.city != "None" && (
              <div className="bg-blue-700 py-2 px-4 text-white rounded-md font-bold flex gap-2 items-center"><MapPin size={12} className="textwhite" /> {props.city}</div>
            )}
            {(props.orcid != '0' && props.orcid != '') && (
              <Link to={`https://orcid.org/${props.orcid}`} target="_blank" className="bg-[#A6CE39] py-2 px-4 text-white rounded-md font-bold flex gap-2 items-center">
                <IdentificationBadge size={12} className="" />
                Orcid: {props.orcid ? props.orcid.replace(/[^\d-]/g, '') : props.orcid}
              </Link>
            )}
            {props.scopus != '' && (
              <Link to={props.scopus} target="_blank" className="bg-[#FF8200] py-2 px-4 text-white rounded-md font-bold flex gap-2 items-center">
                <IdentificationBadge size={12} className="" />
                Scopus
              </Link>
            )}
            {version && (
              <Link to={`https://somos.ufmg.br/professor/${formatString(props.name)}`} target="_blank" className="bg-[#F3A01E] py-2 px-4 text-white rounded-md font-bold flex gap-2 items-center">
                <IdentificationBadge size={12} className="" />
                Somos UFMG
              </Link>
            )}
        ...
        {props.openAPI && (
          <div className="w-full bg-slate-100 dark:bg-neutral-800 px-4 py-2 rounded-md mb-4 flex gap-3 items-center justify-between">
